...
@Component({
  selector: 'page-dgb',
  templateUrl: 'DGB.html',
  providers: [BarcodeScanner, SQLite, MyAppToolbarComponent]
})

export class DGBPage implements OnInit {
  ...
  private myDGBWallets: Array<WalletDetail> = [];
  ...
  createList() {
    ...
    this.sqlite.create(this.config)
      .then((db: SQLiteObject) => {
        db.executeSql('SELECT * FROM DGB_wallet;', []).then((data) => {
          ...
          if (data.rows.length > 0) {
            ...
            try {
              ...
              if (this.platform.is('android')) {
                  ...
                  (success) => {
                    ...
                    if (obj != null) {
                      ...
                      for (i = 0; i < data.rows.length; i++) {
                        let walletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "DGB", address[i], data.rows.item(i).address.trim(), data.rows.item(i).balance, data.rows.item(i).wallet_name, i, this);
                        this.myDGBWallets.push(walletDetail);
                      }
                      this.sumBalance();
                    }
                  },
                ...
                for (i = 0; i < data.rows.length; i++) {
                  let walletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "DGB", data.rows.item(i).address.trim(), data.rows.item(i).address.trim(), data.rows.item(i).balance, data.rows.item(i).wallet_name, i, this);
                  this.myDGBWallets.push(walletDetail);
                }
                this.sumBalance();
              }
  ...
  addItemToAddressesArray(addressObj) {
    try {
      let newWalletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "DGB", addressObj.address, addressObj.encryptedAddress, "0.0", "", 1, this);
      this.myDGBWallets.push(newWalletDetail);
      this.sumBalance();
    }
    catch (ex) {
      /* alert(ex);*/
  ...
  deleteItemAtIndex(index) {
    let deleteAddress = this.myDGBWallets[index].address;
          ...
          if (this.platform.is('android')) {
            myAppPlugin.encrypt(
              (success) => {
                var obj = JSON.parse(success);
                if (obj != null) {
                  db.executeSql("DELETE FROM DGB_wallet WHERE address='" + obj.address + "';", []).then((result) => {
                    if (result.rowsAffected) {
                      this.myDGBWallets.splice(index, 1);
                      this.deleteFromServerDb(deleteAddress, "DGB");
                      this.sumBalance();
                      //this.createList();
                    }
                    else {
            ...
            db.executeSql("DELETE FROM DGB_wallet WHERE address='" + deleteAddress + "';", []).then((result) => {
              if (result.rowsAffected) {
                this.myDGBWallets.splice(index, 1);
                this.sumBalance();
                //this.createList();
  ...
  sumBalance() {
    ...
    var balances = this.myDGBWallets.map(i => Number(i.balance));
  ...
  refresh() {
    ...
    for (var i = 0; i < this.myDGBWallets.length; i++) {
      let walletDetail: WalletDetail = this.myDGBWallets[i];
  ...
  walletDetails(index) {
    var coin = this.myDGBWallets[index];
    ...
    myModal.onDidDismiss(data => {
      if(data.trim()=="") {
        this.myDGBWallets[index].walletName = "NA";
      }
      else{
        this.myDGBWallets[index].walletName = data;
      } 
      this.ChangeDetectorRef.detectChanges();
    });
  }
