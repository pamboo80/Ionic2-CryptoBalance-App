...
/**
 * Generated class for the LskPage page.
 *
...
@Component({
  selector: 'page-lsk',
  templateUrl: 'lsk.html',
})


export class LskPage {
  ...
  private myLSKWallets: Array<WalletDetail> = [];
  ...
  constructor(platform: Platform,
    ...
    /*
     Observable.from(this.myLSKWallets).subscribe(
     function (x) {
     alert('Next: ' + x);
     },
     function (err) {
  ...
  createList() {
    ...
    this.sqlite.create(this.config)
      .then((db: SQLiteObject) => {
        ...
        db.executeSql('SELECT * FROM LSK_wallet;', []).then((data) => {
          ...
          if (data.rows.length > 0) {
            ...
            try {
              ...
              if (this.platform.is('android')) {
                  ...
                  (success) => {
                    ...
                    if (obj != null) {
                      ...
                      for (i = 0; i < data.rows.length; i++) {
                        let walletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "LSK", address[i], data.rows.item(i).address.trim(), data.rows.item(i).balance, data.rows.item(i).wallet_name, i, this);
                        this.myLSKWallets.push(walletDetail);
                      }
                ...
                for (i = 0; i < data.rows.length; i++) {
                  let walletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "LSK", data.rows.item(i).address.trim(), data.rows.item(i).address.trim(), data.rows.item(i).balance, data.rows.item(i).wallet_name, i, this);
                  this.myLSKWallets.push(walletDetail);
                }
                this.sumBalance();
              }
  ...
  addItemToAddressesArray(addressObj) {
    try {
      let newWalletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "LSK", addressObj.address, addressObj.encryptedAddress, "0.0", "", 1, this);
      this.myLSKWallets.push(newWalletDetail);
  ...
  deleteItemAtIndex(index) {
    ...
    let deleteAddress = this.myLSKWallets[index].address;
          ...
          if (this.platform.is('android')) {
            myAppPlugin.encrypt(
              (success) => {
                var obj = JSON.parse(success);
                if (obj != null) {
                  db.executeSql("DELETE FROM LSK_wallet WHERE address='" + obj.address + "';", []).then((result) => {
                    if (result.rowsAffected) {
                      this.myLSKWallets.splice(index, 1);
                      this.sumBalance();
                      //this.createList();
                    }
                    else {
            ...
            db.executeSql("DELETE FROM LSK_wallet WHERE address='" + deleteAddress + "';", []).then((result) => {
              if (result.rowsAffected) {
                this.myLSKWallets.splice(index, 1);
                this.sumBalance();
                //this.createList();
  ...
  sumBalance() {
    ...
    var balances = this.myLSKWallets.map(i => Number(i.balance));
    ...
    //this.totalBalance = this.myLSKWallets.map(i => Number(i.balance)).reduce((a, b) => a + b, 0);
    ...
    /*
     ...
     db.executeSql('SELECT count(*) as recordCount, sum(balance) as totalBalance  FROM LSK_wallet;', []).then((data) => {
     //alert(data.rows.length);
     if (data.rows.length > 0) {
  ...
  refresh() {
    ...
    for (var i = 0; i < this.myLSKWallets.length; i++) {
      let walletDetail: WalletDetail = this.myLSKWallets[i];
  ...
  walletDetails(index) {
    var coin = this.myLSKWallets[index];
    ...
    myModal.onDidDismiss(data => {
      if(data.trim()=="") {
        this.myLSKWallets[index].walletName = "NA";
      }
      else{
        this.myLSKWallets[index].walletName = data;
      } 
      this.ChangeDetectorRef.detectChanges();
    });
  }
