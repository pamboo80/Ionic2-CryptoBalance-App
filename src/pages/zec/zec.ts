...
@Component({
  selector: 'page-zec',
  templateUrl: 'ZEC.html',
  providers: [BarcodeScanner, SQLite, MyAppToolbarComponent]
})

export class ZECPage implements OnInit {
  ...
  private myZECWallets: Array<WalletDetail> = [];
  ...
  createList() {
    ...
    // let walletDetail0: WalletDetail = new WalletDetail(this.http, this.sqlite, "ZEC", "t1e8F9YdPuNKv8JjWspXPxoP8ZJADtwKdQs", "1F1tAaz5x1HUXrCNLbtMDqcw6o5GNn4xqX", "0.0", "", this);
    // this.myZECWallets.push(walletDetail0);
  
    //Read from the DB and create the list
    //create the sqllite db/table
    this.sqlite.create(this.config)
      .then((db: SQLiteObject) => {
        db.executeSql('SELECT * FROM ZEC_wallet;', []).then((data) => {
          ...
          if (data.rows.length > 0) {
            ...
            try {
              ...
              if (this.platform.is('android')) {
                  ...
                  (success) => {
                    ...
                    if (obj != null) {
                      ...
                      for (i = 0; i < data.rows.length; i++) {                        
                        let walletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "ZEC", address[i], data.rows.item(i).address.trim(), data.rows.item(i).balance, data.rows.item(i).wallet_name, i, this);
                        this.myZECWallets.push(walletDetail);
                      }
                      this.sumBalance();
                    }
                  },
                ...
                for (i = 0; i < data.rows.length; i++) {
                  let walletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "ZEC", data.rows.item(i).address.trim(), data.rows.item(i).address.trim(), data.rows.item(i).balance, data.rows.item(i).wallet_name, i, this);
                  this.myZECWallets.push(walletDetail);
                }
                this.sumBalance();
              }
  ...
  addItemToAddressesArray(addressObj) {
    try {
      let newWalletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "ZEC", addressObj.address, addressObj.encryptedAddress, "0.0", "", 1, this);
      this.myZECWallets.push(newWalletDetail);
      this.sumBalance();
    }
    catch (ex) {
      /* alert(ex);*/
  ...
  deleteItemAtIndex(index) {
    let deleteAddress = this.myZECWallets[index].address;
          ...
          if (this.platform.is('android')) {
            myAppPlugin.encrypt(
              (success) => {
                var obj = JSON.parse(success);
                if (obj != null) {
                  db.executeSql("DELETE FROM ZEC_wallet WHERE address='" + obj.address + "';", []).then((result) => {
                    if (result.rowsAffected) {
                      this.myZECWallets.splice(index, 1);
                      this.deleteFromServerDb(deleteAddress, "ZEC");
                      this.sumBalance();
                      //this.createList();
                    }
                    else {
            ...
            db.executeSql("DELETE FROM ZEC_wallet WHERE address='" + deleteAddress + "';", []).then((result) => {
              if (result.rowsAffected) {
                this.myZECWallets.splice(index, 1);
                this.sumBalance();
                //this.createList();
  ...
  sumBalance() {
    ...
    var balances = this.myZECWallets.map(i => Number(i.balance));
  ...
  refresh() {
    ...
    for (var i = 0; i < this.myZECWallets.length; i++) {
      let walletDetail: WalletDetail = this.myZECWallets[i];
  ...
  walletDetails(index) {
    var coin = this.myZECWallets[index];
    ...
    myModal.onDidDismiss(data => {
      if(data.trim()=="") {
        this.myZECWallets[index].walletName = "NA";
      }
      else{
        this.myZECWallets[index].walletName = data;
      } 
      this.ChangeDetectorRef.detectChanges();
    });
  }
