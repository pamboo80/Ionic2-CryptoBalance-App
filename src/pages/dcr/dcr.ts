...
@Component({
  selector: 'page-dcr',
  templateUrl: 'DCR.html',
  providers: [BarcodeScanner, SQLite, MyAppToolbarComponent]
})

export class DCRPage implements OnInit {
  ...
  private myDCRWallets: Array<WalletDetail> = [];
  ...
  createList() {
    ...
    this.sqlite.create(this.config)
      .then((db: SQLiteObject) => {
        db.executeSql('SELECT * FROM DCR_wallet;', []).then((data) => {
          ...
          if (data.rows.length > 0) {
            ...
            try {
              ...
              if (this.platform.is('android')) {
                  ...
                  (success) => {
                    ...
                    if (obj != null) {
                      ...
                      for (i = 0; i < data.rows.length; i++) {
                        let walletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "DCR", address[i], data.rows.item(i).address.trim(), data.rows.item(i).balance, data.rows.item(i).wallet_name, i, this);
                        this.myDCRWallets.push(walletDetail);
                      }
                      this.sumBalance();
                    }
                  },
                ...
                for (i = 0; i < data.rows.length; i++) {
                  let walletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "DCR", data.rows.item(i).address.trim(), data.rows.item(i).address.trim(), data.rows.item(i).balance, data.rows.item(i).wallet_name, i, this);
                  this.myDCRWallets.push(walletDetail);
                }
                this.sumBalance();
              }
  ...
  addItemToAddressesArray(addressObj) {
    try {
      let newWalletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "DCR", addressObj.address, addressObj.encryptedAddress, "0.0", "", 1, this);
      this.myDCRWallets.push(newWalletDetail);
      this.sumBalance();
    }
    catch (ex) {
      /* alert(ex);*/
  ...
  deleteItemAtIndex(index) {
    let deleteAddress = this.myDCRWallets[index].address;
          ...
          if (this.platform.is('android')) {
            myAppPlugin.encrypt(
              (success) => {
                var obj = JSON.parse(success);
                if (obj != null) {
                  db.executeSql("DELETE FROM DCR_wallet WHERE address='" + obj.address + "';", []).then((result) => {
                    if (result.rowsAffected) {
                      this.myDCRWallets.splice(index, 1);
                      this.deleteFromServerDb(deleteAddress, "DCR");
                      this.sumBalance();
                      //this.createList();
                    }
                    else {
            ...
            db.executeSql("DELETE FROM DCR_wallet WHERE address='" + deleteAddress + "';", []).then((result) => {
              if (result.rowsAffected) {
                this.myDCRWallets.splice(index, 1);
                this.sumBalance();
                //this.createList();
  ...
  sumBalance() {
    ...
    var balances = this.myDCRWallets.map(i => Number(i.balance));
  ...
  refresh() {
    ...
    for (var i = 0; i < this.myDCRWallets.length; i++) {
      let walletDetail: WalletDetail = this.myDCRWallets[i];
  ...
  walletDetails(index) {
    var coin = this.myDCRWallets[index];
    ...
    myModal.onDidDismiss(data => {
      if(data.trim()=="") {
        this.myDCRWallets[index].walletName = "NA";
      }
      else{
        this.myDCRWallets[index].walletName = data;
      } 
      this.ChangeDetectorRef.detectChanges();
    });
  }
