...
export class ETCPage implements OnInit {
    ...
    private myETCWallets: Array<WalletDetail> = [];
    ...
    createList() {
        ...
        this.sqlite.create(this.config)
            .then((db: SQLiteObject) => {
                db.executeSql('SELECT * FROM ETC_wallet;', []).then((data) => {
                    ...
                    if (data.rows.length > 0) {
                        ...
                        try {
                            ...
                            if (this.platform.is('android')) {
                                    ...
                                    (success) => {
                                        ...
                                        if (obj != null) {
                                            ...
                                            for (i = 0; i < data.rows.length; i++) {
                                                let walletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "ETC", address[i], data.rows.item(i).address.trim(), data.rows.item(i).balance, data.rows.item(i).wallet_name, i, this);
                                                this.myETCWallets.push(walletDetail);
                                            }
                                            this.sumBalance();
                                        }
                                    },
                                ...
                                for (i = 0; i < data.rows.length; i++) {
                                    let walletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "ETC", data.rows.item(i).address.trim(), data.rows.item(i).address.trim(), data.rows.item(i).balance, data.rows.item(i).wallet_name, i, this);
                                    this.myETCWallets.push(walletDetail);
                                }
                                this.sumBalance();
                            }
    ...
    addItemToAddressesArray(addressObj) {
        try {
            let newWalletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "ETC", addressObj.address, addressObj.encryptedAddress, "0.0", "", 1, this);
    ...
    deleteItemAtIndex(index) {
        let deleteAddress = this.myETCWallets[index].address;
                    ...
                    if (this.platform.is('android')) {
                        myAppPlugin.encrypt(
                            (success) => {
                                var obj = JSON.parse(success);
                                if (obj != null) {
                                    db.executeSql("DELETE FROM ETC_wallet WHERE address='" + obj.address + "';", []).then((result) => {
                                        if (result.rowsAffected) {
                                            this.myETCWallets.splice(index, 1);
                                            this.deleteFromServerDb(deleteAddress, "ETC");
                                            this.sumBalance();
                                            //this.createList();
                                        }
                                        else {
                        ...
                        db.executeSql("DELETE FROM ETC_wallet WHERE address='" + deleteAddress + "';", []).then((result) => {
                            if (result.rowsAffected) {
                                this.myETCWallets.splice(index, 1);
                                this.deleteFromServerDb(deleteAddress, "ETC");
                                this.sumBalance();
    ...
    sumBalance() {
        ...
        var balances = this.myETCWallets.map(i => Number(i.balance));
    ...
    refresh() {
        ...
        for (var i = 0; i < this.myETCWallets.length; i++) {
            let walletDetail: WalletDetail = this.myETCWallets[i];
    ...
    walletDetails(index) {
        var coin = this.myETCWallets[index];
        ...
        myModal.onDidDismiss(data => {
            if(data.trim()=="") {
              this.myETCWallets[index].walletName = "NA";
            }
            else{
              this.myETCWallets[index].walletName = data;
            } 
            this.ChangeDetectorRef.detectChanges();
          });
    }
