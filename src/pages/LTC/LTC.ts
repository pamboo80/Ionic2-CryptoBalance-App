...
@Component({
    selector: 'page-ltc',
    templateUrl: 'LTC.html',
    providers: [BarcodeScanner, SQLite, MyAppToolbarComponent]
})

export class LTCPage implements OnInit {
    token: String;
    email: String;
    uuid: String;
    ...
    private myLTCWallets: Array<WalletDetail> = [];
    ...
    createList() {
        ...
        this.sqlite.create(this.config)
            .then((db: SQLiteObject) => {
                db.executeSql('SELECT * FROM LTC_wallet;', []).then((data) => {
                    ...
                    if (data.rows.length > 0) {
                        ...
                        try {
                            ...
                            if (this.platform.is('android')) {
                                    ...
                                    (success) => {
                                        ...
                                        if (obj != null) {
                                            ...
                                            for (i = 0; i < data.rows.length; i++) {
                                                let walletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "LTC", address[i], data.rows.item(i).address.trim(), data.rows.item(i).balance, data.rows.item(i).wallet_name, i, this);
                                                this.myLTCWallets.push(walletDetail);
                                            }
                                            this.sumBalance();
                                        }
                                    },
                                ...
                                for (i = 0; i < data.rows.length; i++) {
                                    let walletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "LTC", data.rows.item(i).address.trim(), data.rows.item(i).address.trim(), data.rows.item(i).balance, data.rows.item(i).wallet_name, i, this);
                                    this.myLTCWallets.push(walletDetail);
                                }
                                this.sumBalance();
                            }
    ...
    addItemToAddressesArray(addressObj) {
        try {
            let newWalletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "LTC", addressObj.address, addressObj.encryptedAddress, "0.0", "", 1, this);
            this.myLTCWallets.push(newWalletDetail);
            this.sumBalance();
        }
        catch (ex) {
            /* alert(ex);*/
    ...
    deleteItemAtIndex(index) {
        let deleteAddress = this.myLTCWallets[index].address;
                    ...
                    if (this.platform.is('android')) {
                        myAppPlugin.encrypt(
                            (success) => {
                                var obj = JSON.parse(success);
                                if (obj != null) {
                                    db.executeSql("DELETE FROM LTC_wallet WHERE address='" + obj.address + "';", []).then((result) => {
                                        if (result.rowsAffected) {
                                            this.myLTCWallets.splice(index, 1);
                                            this.deleteFromServerDb(deleteAddress, "LTC");
                                            this.sumBalance();
                                            //this.createList();
                                        }
                                        else {
                        ...
                        db.executeSql("DELETE FROM LTC_wallet WHERE address='" + deleteAddress + "';", []).then((result) => {
                            if (result.rowsAffected) {
                                this.myLTCWallets.splice(index, 1);
                                this.deleteFromServerDb(deleteAddress, "LTC");
                                this.sumBalance();
    ...
    sumBalance() {
        ...
        var balances = this.myLTCWallets.map(i => Number(i.balance));
    ...
    refresh() {
        ...
        for (var i = 0; i < this.myLTCWallets.length; i++) {
            let walletDetail: WalletDetail = this.myLTCWallets[i];
    ...
    walletDetails(index) {
        var coin = this.myLTCWallets[index];
        ...
        myModal.onDidDismiss(data => {
            if (data.trim() == "") {
                this.myLTCWallets[index].walletName = "NA";
            }
            else {
                this.myLTCWallets[index].walletName = data;
            }
            this.ChangeDetectorRef.detectChanges();
        });
    }
        ...
        this.storage.get("email").then((val) => {
            ...
            if (this.email == null || this.email == "") {
                ...
                this.storage.get("uuid").then((val) => {
                    ...
                    // let URL = this.domain + coinType + "/" + this.uuid + "/" + address;
