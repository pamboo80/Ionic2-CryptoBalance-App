...
@Component({
  selector: 'page-sia',
  templateUrl: 'sia.html',
  providers: [BarcodeScanner, SQLite, MyAppToolbarComponent]
})

export class SIAPage implements OnInit {
  ...
  private mySIAWallets: Array<WalletDetail> = [];
  ...
  createList() {
    ...
    this.sqlite.create(this.config)
      .then((db: SQLiteObject) => {
        db.executeSql('SELECT * FROM SC_Wallet;', []).then((data) => {
          ...
          if (data.rows.length > 0) {
            ...
            try {
              ...
              if (this.platform.is('android')) {
                  ...
                  (success) => {
                    ...
                    if (obj != null) {
                      ...
                      for (i = 0; i < data.rows.length; i++) {
                        let walletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "SC", address[i], data.rows.item(i).address.trim(), data.rows.item(i).balance, data.rows.item(i).wallet_name, i,  this);
                        this.mySIAWallets.push(walletDetail);
                      }
                      this.sumBalance();
                    }
                  },
                ...
                for (i = 0; i < data.rows.length; i++) {
                  let walletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "SC", data.rows.item(i).address.trim(), data.rows.item(i).address.trim(), data.rows.item(i).balance, data.rows.item(i).wallet_name, i,  this);
                  this.mySIAWallets.push(walletDetail);
                }
                this.sumBalance();
              }
  ...
  addItemToAddressesArray(addressObj) {
    try {
      let newWalletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "SC", addressObj.address, addressObj.encryptedAddress, "0.0", "", 1, this);
      this.mySIAWallets.push(newWalletDetail);
      this.sumBalance();
    }
    catch (ex) {
      /* alert(ex);*/
  ...
  deleteItemAtIndex(index) {
    let deleteAddress = this.mySIAWallets[index].address;
          ...
          if (this.platform.is('android')) {
            myAppPlugin.encrypt(
              (success) => {
                var obj = JSON.parse(success);
                if (obj != null) {
                  db.executeSql("DELETE FROM SC_Wallet WHERE address='" + obj.address + "';", []).then((result) => {
                    if (result.rowsAffected) {
                      this.mySIAWallets.splice(index, 1);
                      this.deleteFromServerDb(deleteAddress, "SC");
                      this.sumBalance();
                      //this.createList();
                    }
                    else {
            ...
            db.executeSql("DELETE FROM SC_Wallet WHERE address='" + deleteAddress + "';", []).then((result) => {
              if (result.rowsAffected) {
                this.mySIAWallets.splice(index, 1);
                this.sumBalance();
                //this.createList();
  ...
  sumBalance() {
    ...
    var balances = this.mySIAWallets.map(i => Number(i.balance));
  ...
  refresh() {
    ...
    for (var i = 0; i < this.mySIAWallets.length; i++) {
      let walletDetail: WalletDetail = this.mySIAWallets[i];
  ...
  walletDetails(index) {
    var coin = this.mySIAWallets[index];
    ...
    myModal.onDidDismiss(data => {
      if(data.trim()=="") {
        this.mySIAWallets[index].walletName = "NA";
      }
      else{
        this.mySIAWallets[index].walletName = data;
      } 
      this.ChangeDetectorRef.detectChanges();
    });
  }
