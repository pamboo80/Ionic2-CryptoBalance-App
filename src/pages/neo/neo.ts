...
/**
 * Generated class for the NeoPage page.
 *
...
export class NeoPage {
  ...
  private myNEOWallets: Array<WalletDetail> = [];
  ...
  constructor(platform: Platform,
    ...
    /*
     Observable.from(this.myNEOWallets).subscribe(
     function (x) {
     alert('Next: ' + x);
     },
     function (err) {
  ...
  createList() {
    ...
    this.sqlite.create(this.config)
      .then((db: SQLiteObject) => {
        ...
        db.executeSql('SELECT * FROM NEO_wallet;', []).then((data) => {
          ...
          if (data.rows.length > 0) {
            ...
            try {
              ...
              if (this.platform.is('android')) {
                  ...
                  (success) => {
                    ...
                    if (obj != null) {
                      ...
                      for (i = 0; i < data.rows.length; i++) {
                        let walletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "NEO", address[i], data.rows.item(i).address.trim(), data.rows.item(i).balance, data.rows.item(i).wallet_name, i, this);
                        this.myNEOWallets.push(walletDetail);
                      }
                ...
                for (i = 0; i < data.rows.length; i++) {
                  let walletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "NEO", data.rows.item(i).address.trim(), data.rows.item(i).address.trim(), data.rows.item(i).balance, data.rows.item(i).wallet_name, i,this);
                  this.myNEOWallets.push(walletDetail);
                }
                this.sumBalance();
              }
  ...
  addItemToAddressesArray(addressObj) {
    try {
      let newWalletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "NEO", addressObj.address, addressObj.encryptedAddress, "0.0", "", 1, this);
      this.myNEOWallets.push(newWalletDetail);
  ...
  deleteItemAtIndex(index) {
    ...
    let deleteAddress = this.myNEOWallets[index].address;
          ...
          if (this.platform.is('android')) {
            myAppPlugin.encrypt(
              (success) => {
                var obj = JSON.parse(success);
                if (obj != null) {
                  db.executeSql("DELETE FROM NEO_wallet WHERE address='" + obj.address + "';", []).then((result) => {
                    if (result.rowsAffected) {
                      this.myNEOWallets.splice(index, 1);
                      this.sumBalance();
                      //this.createList();
                    }
                    else {
            ...
            db.executeSql("DELETE FROM NEO_wallet WHERE address='" + deleteAddress + "';", []).then((result) => {
              if (result.rowsAffected) {
                this.myNEOWallets.splice(index, 1);
                this.sumBalance();
                //this.createList();
  ...
  sumBalance() {
    ...
    var balances = this.myNEOWallets.map(i => Number(i.balance));
    ...
    //this.totalBalance = this.myNEOWallets.map(i => Number(i.balance)).reduce((a, b) => a + b, 0);
    ...
    /*
     ...
     db.executeSql('SELECT count(*) as recordCount, sum(balance) as totalBalance  FROM NEO_wallet;', []).then((data) => {
     //alert(data.rows.length);
     if (data.rows.length > 0) {
  ...
  refresh() {
    ...
    for (var i = 0; i < this.myNEOWallets.length; i++) {
      let walletDetail: WalletDetail = this.myNEOWallets[i];
  ...
  walletDetails(index) {
    var coin = this.myNEOWallets[index];
    ...
    myModal.onDidDismiss(data => {
      if(data.trim()=="") {
        this.myNEOWallets[index].walletName = "NA";
      }
      else{
        this.myNEOWallets[index].walletName = data;
      } 
      this.ChangeDetectorRef.detectChanges();
    });
  }
