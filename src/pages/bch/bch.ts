...
/**
 * Generated class for the BchPage page.
 *
...
@Component({
  selector: 'page-bch',
  templateUrl: 'bch.html',
})
export class BchPage {
  ...
  private myBCHWallets: Array<WalletDetail> = [];
  ...
  constructor(platform: Platform,
    ...
    /*
     Observable.from(this.myBCHWallets).subscribe(
     function (x) {
     alert('Next: ' + x);
     },
     function (err) {
  ...
  createList() {
    ...
    // //Supports only legacy address format only. 
    // let walletDetail0: WalletDetail = new WalletDetail(this.http, this.sqlite, "BCH", "1FbBbv5oYqFKwiPm4CAqvAy8345n8AQ74b", "1F1tAaz5x1HUXrCNLbtMDqcw6o5GNn4xqX", "0.0", "", this);
    // this.myBCHWallets.push(walletDetail0);
    // //The API doesn't support the new address format
    // let walletDetail1: WalletDetail = new WalletDetail(this.http, this.sqlite, "BCH", "qzsq3s8k6cpkd9r35y7pezssrf4pcuayavs4ne7ecc", "1F1tAaz5x1HUXrCNLbtMDqcw6o5GNn4xqX", "0.0", "", this);    
    // this.myBCHWallets.push(walletDetail1);
    ...
    this.sqlite.create(this.config)
      .then((db: SQLiteObject) => {
        ...
        db.executeSql('SELECT * FROM BCH_wallet;', []).then((data) => {
          ...
          if (data.rows.length > 0) {
            ...
            try {
              ...
              if (this.platform.is('android')) {
                  ...
                  (success) => {
                    ...
                    if (obj != null) {
                      ...
                      for (i = 0; i < data.rows.length; i++) {
                        let walletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "BCH", address[i], data.rows.item(i).address.trim(), data.rows.item(i).balance, data.rows.item(i).wallet_name, i,this);
                        this.myBCHWallets.push(walletDetail);
                      }
                ...
                for (i = 0; i < data.rows.length; i++) {
                  let walletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "BCH", data.rows.item(i).address.trim(), data.rows.item(i).address.trim(), data.rows.item(i).balance, data.rows.item(i).wallet_name, i, this);
                  this.myBCHWallets.push(walletDetail);
                }
                this.sumBalance();
              }
  ...
  addItemToAddressesArray(addressObj) {
    try {
      let newWalletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "BCH", addressObj.address, addressObj.encryptedAddress, "0.0", "", 1, this);
      this.myBCHWallets.push(newWalletDetail);
  ...
  deleteItemAtIndex(index) {
    ...
    let deleteAddress = this.myBCHWallets[index].address;
          ...
          if (this.platform.is('android')) {
            myAppPlugin.encrypt(
              (success) => {
                var obj = JSON.parse(success);
                if (obj != null) {
                  db.executeSql("DELETE FROM BCH_wallet WHERE address='" + obj.address + "';", []).then((result) => {
                    if (result.rowsAffected) {
                      this.myBCHWallets.splice(index, 1);
                      this.deleteFromServerDb(deleteAddress, "BCH");
                      this.sumBalance();
                      //this.createList();
                    }
                    else {
            ...
            db.executeSql("DELETE FROM BCH_wallet WHERE address='" + deleteAddress + "';", []).then((result) => {
              if (result.rowsAffected) {
                this.myBCHWallets.splice(index, 1);
                this.sumBalance();
                //this.createList();
  ...
  sumBalance() {
    ...
    var balances = this.myBCHWallets.map(i => Number(i.balance));
    ...
    //this.totalBalance = this.myBCHWallets.map(i => Number(i.balance)).reduce((a, b) => a + b, 0);
    ...
    /*
     ...
     db.executeSql('SELECT count(*) as recordCount, sum(balance) as totalBalance  FROM BCH_wallet;', []).then((data) => {
     //alert(data.rows.length);
     if (data.rows.length > 0) {
  ...
  refresh() {
    ...
    for (var i = 0; i < this.myBCHWallets.length; i++) {
      let walletDetail: WalletDetail = this.myBCHWallets[i];
  ...
  walletDetails(index) {
    var coin = this.myBCHWallets[index];
    ...
    myModal.onDidDismiss(data => {
      if(data.trim()=="") {
        this.myBCHWallets[index].walletName = "NA";
      }
      else{
        this.myBCHWallets[index].walletName = data;
      } 
      this.ChangeDetectorRef.detectChanges();
    });
  }
