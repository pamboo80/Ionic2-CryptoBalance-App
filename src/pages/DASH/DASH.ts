...
export class DASHPage {
  ...
  private myDASHWallets: Array<WalletDetail> = [];
  ...
  constructor(platform: Platform,
    ...
    /*
     Observable.from(this.myDASHWallets).subscribe(
     function (x) {
     alert('Next: ' + x);
     },
     function (err) {
  ...
  createList() {
    ...
    this.sqlite.create(this.config)
      .then((db: SQLiteObject) => {
        ...
        db.executeSql('SELECT * FROM DASH_wallet;', []).then((data) => {
          ...
          if (data.rows.length > 0) {
            ...
            try {
              ...
              if (this.platform.is('android')) {
                  ...
                  (success) => {
                    ...
                    if (obj != null) {
                      ...
                      for (i = 0; i < data.rows.length; i++) {
                        let walletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "DASH", address[i], data.rows.item(i).address.trim(), data.rows.item(i).balance, data.rows.item(i).wallet_name, i, this);
                        this.myDASHWallets.push(walletDetail);
                      }
                ...
                for (i = 0; i < data.rows.length; i++) {
                  let walletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "DASH", data.rows.item(i).address.trim(), data.rows.item(i).address.trim(), data.rows.item(i).balance, data.rows.item(i).wallet_name, i, this);
                  this.myDASHWallets.push(walletDetail);
                }
                this.sumBalance();
              }
  ...
  addItemToAddressesArray(addressObj) {
    try {
      let newWalletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "DASH", addressObj.address, addressObj.encryptedAddress, "0.0", "", 1, this);
      this.myDASHWallets.push(newWalletDetail);
  ...
  deleteItemAtIndex(index) {
    ...
    let deleteAddress = this.myDASHWallets[index].address;
          ...
          if (this.platform.is('android')) {
            myAppPlugin.encrypt(
              (success) => {
                var obj = JSON.parse(success);
                if (obj != null) {
                  db.executeSql("DELETE FROM DASH_wallet WHERE address='" + obj.address + "';", []).then((result) => {
                    if (result.rowsAffected) {
                      this.myDASHWallets.splice(index, 1);
                      this.sumBalance();
                      //this.createList();
                    }
                    else {
            ...
            db.executeSql("DELETE FROM DASH_wallet WHERE address='" + deleteAddress + "';", []).then((result) => {
              if (result.rowsAffected) {
                this.myDASHWallets.splice(index, 1);
                this.sumBalance();
                //this.createList();
  ...
  sumBalance() {
    ...
    var balances = this.myDASHWallets.map(i => Number(i.balance));
    ...
    //this.totalBalance = this.myDASHWallets.map(i => Number(i.balance)).reduce((a, b) => a + b, 0);
    ...
    /*
     ...
     db.executeSql('SELECT count(*) as recordCount, sum(balance) as totalBalance  FROM DASH_wallet;', []).then((data) => {
     //alert(data.rows.length);
     if (data.rows.length > 0) {
  ...
  refresh() {
    ...
    for (var i = 0; i < this.myDASHWallets.length; i++) {
      let walletDetail: WalletDetail = this.myDASHWallets[i];
  ...
  walletDetails(index) {
    var coin = this.myDASHWallets[index];
    ...
    myModal.onDidDismiss(data => {
      if(data.trim()=="") {
        this.myDASHWallets[index].walletName = "NA";
      }
      else{
        this.myDASHWallets[index].walletName = data;
      } 
      this.ChangeDetectorRef.detectChanges();
    });
  }
