...
/**
 * Generated class for the NxtPage page.
 *
...
export class NXTPage {
  ...
  private myNXTWallets: Array<WalletDetail> = [];
  ...
  constructor(platform: Platform,
    ...
    /*
     Observable.from(this.myNXTWallets).subscribe(
     function (x) {
     alert('Next: ' + x);
     },
     function (err) {
  ...
  createList() {
    ...
    this.sqlite.create(this.config)
      .then((db: SQLiteObject) => {
        ...
        db.executeSql('SELECT * FROM NXT_wallet;', []).then((data) => {
          ...
          if (data.rows.length > 0) {
            ...
            try {
              ...
              if (this.platform.is('android')) {
                  ...
                  (success) => {
                    ...
                    if (obj != null) {
                      ...
                      for (i = 0; i < data.rows.length; i++) {
                        let walletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "NXT", address[i], data.rows.item(i).address.trim(), data.rows.item(i).balance, data.rows.item(i).wallet_name, i, this);
                        this.myNXTWallets.push(walletDetail);
                      }
                ...
                for (i = 0; i < data.rows.length; i++) {
                  let walletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "NXT", data.rows.item(i).address.trim(), data.rows.item(i).address.trim(), data.rows.item(i).balance, data.rows.item(i).wallet_name, i,  this);
                  this.myNXTWallets.push(walletDetail);
                }
                this.sumBalance();
              }
  ...
  addItemToAddressesArray(addressObj) {
    try {
      let newWalletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "NXT", addressObj.address, addressObj.encryptedAddress, "0.0", "", 1, this);
      this.myNXTWallets.push(newWalletDetail);
  ...
  deleteItemAtIndex(index) {
    ...
    let deleteAddress = this.myNXTWallets[index].address;
          ...
          if (this.platform.is('android')) {
            myAppPlugin.encrypt(
              (success) => {
                var obj = JSON.parse(success);
                if (obj != null) {
                  db.executeSql("DELETE FROM NXT_wallet WHERE address='" + obj.address + "';", []).then((result) => {
                    if (result.rowsAffected) {
                      this.myNXTWallets.splice(index, 1);
                      this.sumBalance();
                      //this.createList();
                    }
                    else {
            ...
            db.executeSql("DELETE FROM NXT_wallet WHERE address='" + deleteAddress + "';", []).then((result) => {
              if (result.rowsAffected) {
                this.myNXTWallets.splice(index, 1);
                this.sumBalance();
                //this.createList();
  ...
  sumBalance() {
    ...
    var balances = this.myNXTWallets.map(i => Number(i.balance));
    ...
    //this.totalBalance = this.myNXTWallets.map(i => Number(i.balance)).reduce((a, b) => a + b, 0);
    ...
    /*
     ...
     db.executeSql('SELECT count(*) as recordCount, sum(balance) as totalBalance  FROM NXT_wallet;', []).then((data) => {
     //alert(data.rows.length);
     if (data.rows.length > 0) {
  ...
  refresh() {
    ...
    for (var i = 0; i < this.myNXTWallets.length; i++) {
      let walletDetail: WalletDetail = this.myNXTWallets[i];
  ...
  walletDetails(index) {
    var coin = this.myNXTWallets[index];
    ...
    myModal.onDidDismiss(data => {
      if(data.trim()=="") {
        this.myNXTWallets[index].walletName = "NA";
      }
      else{
        this.myNXTWallets[index].walletName = data;
      } 
      this.ChangeDetectorRef.detectChanges();
    });
  }
