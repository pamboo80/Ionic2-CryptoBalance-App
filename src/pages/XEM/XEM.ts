...
/**
 * Generated class for the XemPage page.
 *
...
export class XEMPage {
  ...
  private myXEMWallets: Array<WalletDetail> = [];
  ...
  constructor(platform: Platform,
    ...
    /*
     Observable.from(this.myXEMWallets).subscribe(
     function (x) {
     alert('Next: ' + x);
     },
     function (err) {
  ...
  createList() {
    ...
    this.sqlite.create(this.config)
      .then((db: SQLiteObject) => {
        ...
        db.executeSql('SELECT * FROM XEM_wallet;', []).then((data) => {
          ...
          if (data.rows.length > 0) {
            ...
            try {
              ...
              if (this.platform.is('android')) {
                  ...
                  (success) => {
                    ...
                    if (obj != null) {
                      ...
                      for (i = 0; i < data.rows.length; i++) {
                        let walletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "XEM", address[i], data.rows.item(i).address.trim(), data.rows.item(i).balance, data.rows.item(i).wallet_name, i, this);
                        this.myXEMWallets.push(walletDetail);
                      }
                ...
                for (i = 0; i < data.rows.length; i++) {
                  let walletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "XEM", data.rows.item(i).address.trim(), data.rows.item(i).address.trim(), data.rows.item(i).balance, data.rows.item(i).wallet_name, i, this);
                  this.myXEMWallets.push(walletDetail);
                }
                this.sumBalance();
              }
  ...
  addItemToAddressesArray(addressObj) {
    try {
      let newWalletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "XEM", addressObj.address, addressObj.encryptedAddress, "0.0", "", 1, this);
      this.myXEMWallets.push(newWalletDetail);
  ...
  deleteItemAtIndex(index) {
    ...
    let deleteAddress = this.myXEMWallets[index].address;
          ...
          if (this.platform.is('android')) {
            myAppPlugin.encrypt(
              (success) => {
                var obj = JSON.parse(success);
                if (obj != null) {
                  db.executeSql("DELETE FROM XEM_wallet WHERE address='" + obj.address + "';", []).then((result) => {
                    if (result.rowsAffected) {
                      this.myXEMWallets.splice(index, 1);
                      this.sumBalance();
                      //this.createList();
                    }
                    else {
            ...
            db.executeSql("DELETE FROM XEM_wallet WHERE address='" + deleteAddress + "';", []).then((result) => {
              if (result.rowsAffected) {
                this.myXEMWallets.splice(index, 1);
                this.sumBalance();
                //this.createList();
  ...
  sumBalance() {
    ...
    var balances = this.myXEMWallets.map(i => Number(i.balance));
    ...
    //this.totalBalance = this.myXEMWallets.map(i => Number(i.balance)).reduce((a, b) => a + b, 0);
    ...
    /*
     ...
     db.executeSql('SELECT count(*) as recordCount, sum(balance) as totalBalance  FROM XEM_wallet;', []).then((data) => {
     //alert(data.rows.length);
     if (data.rows.length > 0) {
  ...
  refresh() {
    ...
    for (var i = 0; i < this.myXEMWallets.length; i++) {
      let walletDetail: WalletDetail = this.myXEMWallets[i];
  ...
  walletDetails(index) {
    var coin = this.myXEMWallets[index];
    ...
    myModal.onDidDismiss(data => {
      if(data.trim()=="") {
        this.myXEMWallets[index].walletName = "NA";
      }
      else{
        this.myXEMWallets[index].walletName = data;
      } 
      this.ChangeDetectorRef.detectChanges();
    });
  }
