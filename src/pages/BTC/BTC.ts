...
export class BTCPage implements OnInit {
  ...
  private myBTCWallets: Array<WalletDetail> = [];
  ...
  createList() {
    ...
    // let walletDetail0: WalletDetail = new WalletDetail(this.http, this.sqlite, "BTC", "1F1tAaz5x1HUXrCNLbtMDqcw6o5GNn4xqX", "1F1tAaz5x1HUXrCNLbtMDqcw6o5GNn4xqX", "0.0", "", this);
    // this.myBTCWallets.push(walletDetail0);
    ...
    this.sqlite.create(this.config)
      .then((db: SQLiteObject) => {
        db.executeSql('SELECT * FROM BTC_wallet;', []).then((data) => {
          ...
          if (data.rows.length > 0) {
            ...
            try {
              ...
              if (this.platform.is('android')) {
                  ...
                  (success) => {
                    ...
                    if (obj != null) {
                      ...
                      for (i = 0; i < data.rows.length; i++) {
                        let walletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "BTC", address[i], data.rows.item(i).address.trim(), data.rows.item(i).balance, data.rows.item(i).wallet_name, i, this);
                        this.myBTCWallets.push(walletDetail);
                      }
                      this.sumBalance();
                    }
                  },
                ...
                for (i = 0; i < data.rows.length; i++) {
                  let walletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "BTC", data.rows.item(i).address.trim(), data.rows.item(i).address.trim(), data.rows.item(i).balance, data.rows.item(i).wallet_name, i, this);
                  this.myBTCWallets.push(walletDetail);
                }
                this.sumBalance();
              }
  ...
  addItemToAddressesArray(addressObj) {
    try {
      let newWalletDetail: WalletDetail = new WalletDetail(this.http, this.sqlite, "BTC", addressObj.address, addressObj.encryptedAddress, "0.0", "", 1, this);
  ...
  deleteItemAtIndex(index) {
    let deleteAddress = this.myBTCWallets[index].address;
    try {
      this.sqlite.create(this.config)
        .then((db: SQLiteObject) => {
          ...
          if (this.platform.is('android')) {
            myAppPlugin.encrypt(
              (success) => {
                var obj = JSON.parse(success);
                if (obj != null) {
                  db.executeSql("DELETE FROM BTC_wallet WHERE address='" + obj.address + "';", []).then((result) => {
                    if (result.rowsAffected) {
                      this.myBTCWallets.splice(index, 1);
                      this.deleteFromServerDb(deleteAddress, "BTC");
                      this.sumBalance();
                      //this.createList();
                    }
                    else {
            ...
            db.executeSql("DELETE FROM BTC_wallet WHERE address='" + deleteAddress + "';", []).then((result) => {
              if (result.rowsAffected) {
                this.myBTCWallets.splice(index, 1);
                this.deleteFromServerDb(deleteAddress, "BTC");
                this.sumBalance();
  ...
  sumBalance() {
    ...
    var balances = this.myBTCWallets.map(i => Number(i.balance));
  ...
  refresh() {
    ...
    for (var i = 0; i < this.myBTCWallets.length; i++) {
      let walletDetail: WalletDetail = this.myBTCWallets[i];
  ...
  walletDetails(index) {
    var coin = this.myBTCWallets[index];
    ...
    myModal.onDidDismiss(data => {
      if(data.trim()=="") {
        this.myBTCWallets[index].walletName = "NA";
      }
      else{
        this.myBTCWallets[index].walletName = data;
      }     
      this.ChangeDetectorRef.detectChanges();
    });
